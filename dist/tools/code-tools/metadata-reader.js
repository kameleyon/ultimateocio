...
function activate() {
    console.error("[TOOL] metadata-reader activated");
...
function checkExternalTools() {
    ...
    try {
        ...
        console.error('[Metadata Reader] ExifTool is available');
    }
    catch (err) {
        console.error('[Metadata Reader] ExifTool is not available');
    }
    // Check for ImageMagick
    try {
        (0, child_process_1.execSync)('convert -version', { stdio: 'ignore' });
        externalTools.imagemagick = true;
        console.error('[Metadata Reader] ImageMagick is available');
    }
    catch (err) {
        console.error('[Metadata Reader] ImageMagick is not available');
    }
    // Check for ffmpeg
    try {
        (0, child_process_1.execSync)('ffmpeg -version', { stdio: 'ignore' });
        externalTools.ffmpeg = true;
        console.error('[Metadata Reader] FFmpeg is available');
    }
    catch (err) {
        console.error('[Metadata Reader] FFmpeg is not available');
    }
}
/**
...
function onSessionStart(session) {
    console.error(`[Metadata Reader] Session started: ${session.id}`);
}
/**
 ...
 */
async function getExtendedMetadata(filePath, type, useExternalTools = true) {
    const metadata = {};
    try {
        if (type === 'image') {
 ...
 */
async function handleReadMetadata(args) {
    try {
 ...
 */
async function handleExtractMetadata(args) {
    try {
 ...
 */
async function findFiles(dir, results, options) {
 ...
 */
async function handleParseFileContent(args) {
    try {
