...
exports.ConvertFormatSchema = exports.DetectFormatSchema = exports.FormatFileSchema = exports.FormatContentSchema = void 0;
...
async function loadDependencies() {
    try {
        ...
        for (const path of possiblePaths) {
            try {
                prettier = await Promise.resolve(`${path}`).then(s => __importStar(require(s)));
                console.error(`[Formatter] Prettier successfully loaded from ${path}`);
                break;
            }
            catch (e) {
        ...
        if (!prettier) {
            console.error('[Formatter] prettier not available. Some formatting features will be limited.');
        }
    }
    catch (err) {
        console.error('[Formatter] prettier not available. Some formatting features will be limited.');
    ...
    try {
        ...
        for (const path of possiblePaths) {
            try {
                jsBeautify = await Promise.resolve(`${path}`).then(s => __importStar(require(s)));
                console.error(`[Formatter] js-beautify successfully loaded from ${path}`);
                break;
            }
            catch (e) {
                // Continue trying other paths
        ...
        if (jsBeautify) {
            ...
            console.error('[Formatter] js-beautify not available. Some formatting features will be limited.');
        }
    }
    catch (err) {
        console.error('[Formatter] js-beautify not available. Some formatting features will be limited.');
...
function activate() {
    console.error("[TOOL] formatter activated");
...
function onFileWrite(event) {
    console.error(`[Formatter] File write detected: ${event.path}`);
...
function onSessionStart(session) {
    console.error(`[Formatter] Session started: ${session.id}`);
}
/**
...
exports.FormatContentSchema = zod_1.z.object({
    ...
    prettier: zod_1.z.boolean().optional().default(true),
});
exports.FormatFileSchema = zod_1.z.object({
    path: zod_1.z.string(),
    language: zod_1.z.enum([
        'json', 'javascript', 'typescript', 'html', 'css', 'markdown',
        'yaml', 'xml', 'sql', 'graphql', 'python', 'java', 'c', 'cpp',
    ...
    options: zod_1.z.record(zod_1.z.any()).optional(),
    prettier: zod_1.z.boolean().optional().default(true),
    write: zod_1.z.boolean().optional().default(true),
});
exports.DetectFormatSchema = zod_1.z.object({
...
exports.ConvertFormatSchema = zod_1.z.object({
    content: zod_1.z.string().optional(),
    path: zod_1.z.string().optional(),
    fromFormat: zod_1.z.enum(['json', 'yaml', 'xml', 'csv', 'html', 'markdown']),
    toFormat: zod_1.z.enum(['json', 'yaml', 'xml', 'csv', 'html', 'markdown']),
    options: zod_1.z.record(zod_1.z.any()).optional(),
    write: zod_1.z.boolean().optional().default(false),
...
function ensureDependencies() {
    ...
    if (missingDeps.length > 0) {
        console.error(`[Formatter] Missing dependencies: ${missingDeps.join(', ')}`);
        console.error(`[Formatter] Install using: npm install ${missingDeps.join(' ')}`);
    }
}
/**
 ...
 */
async function formatCode(content, language, options) {
 ...
 */
async function handleFormatContent(args) {
    try {
        const result = exports.FormatContentSchema.safeParse(args);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
 ...
 */
async function handleFormatFile(args) {
    try {
        const result = exports.FormatFileSchema.safeParse(args);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
 ...
 */
async function handleDetectFormat(args) {
    try {
        const result = exports.DetectFormatSchema.safeParse(args);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
 ...
 */
async function handleConvertFormat(args) {
    try {
        const result = exports.ConvertFormatSchema.safeParse(args);
        if (!result.success) {
            return {
                content: [{
                        type: "text",
