...
function activate() {
    console.error("[TOOL] cache-cleaner activated");
}
/**
 * Handles file write events for cache tracking
 */
function onFileWrite(event) {
    console.error(`[Cache Cleaner] Watching file write: ${event.path}`);
    // Check if cache size is becoming too large
    if (event.path.includes('cache') || event.path.includes('temp')) {
        console.error(`[Cache Cleaner] Detected cache file change: ${event.path}`);
        monitorCacheSize();
    }
}
/**
...
function onSessionStart(session) {
    console.error(`[Cache Cleaner] Session started: ${session.id}`);
...
async function monitorCacheSize() {
    try {
        ...
        if (totalSize > 500 * 1024 * 1024) {
            console.error(`[Cache Cleaner] Cache size (${formatBytes(totalSize)}) exceeds threshold, auto-cleaning...`);
 ...
 */
async function getCacheDirStats(directory) {
    let totalSize = 0;
    let fileCount = 0;
    try {
 ...
 */
async function handleCleanCache(args) {
    try {
 ...
 */
async function handleAnalyzeCache(args) {
    try {
 ...
 */
async function handleOptimizeMemory(args) {
    try {
 ...
 */
async function handleCacheStatus(args) {
    try {
