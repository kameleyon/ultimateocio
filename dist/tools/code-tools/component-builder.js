...
function activate() {
    console.error("[TOOL] component-builder activated");
...
function detectProjectFramework() {
    try {
        ...
        if (fsSync.existsSync('package.json')) {
            ...
            console.error(`[Component Builder] Detected ${detectedFramework.framework} framework`);
            if (detectedFramework.typescript) {
                console.error('[Component Builder] TypeScript support detected');
            }
            if (detectedFramework.styling.length > 0) {
                console.error(`[Component Builder] Styling libraries: ${detectedFramework.styling.join(', ')}`);
            }
            if (detectedFramework.testingLibraries.length > 0) {
                console.error(`[Component Builder] Testing libraries: ${detectedFramework.testingLibraries.join(', ')}`);
            }
        }
    }
    catch (error) {
...
function onSessionStart(session) {
    console.error(`[Component Builder] Session started: ${session.id}`);
 ...
 */
async function handleAddComponentPart(args) {
    try {
 ...
 */
async function handleAnalyzeComponent(args) {
    try {
