...
function activate() {
    console.error("[TOOL] cli-prompter activated");
}
/**
 * Handles file write events that might trigger CLI prompt updates
 */
function onFileWrite(event) {
    console.error(`[CLI Prompter] Watching file write: ${event.path}`);
    ...
    if (path.basename(event.path).includes('wizard') || path.basename(event.path).includes('prompt')) {
        console.error(`[CLI Prompter] Detected wizard config file change: ${event.path}`);
...
function onSessionStart(session) {
    console.error(`[CLI Prompter] Session started: ${session.id}`);
}
/**
 ...
 */
async function handleShowPrompt(args) {
    try {
 ...
 */
async function handleRunWizard(args) {
    try {
        ...
        if (saveResultsTo) {
            ...
            console.error(`[CLI Prompter] Would save wizard results to: ${saveResultsTo}`);
        }
        // Execute completion callback if provided
        if (onComplete) {
            // In a real implementation, this would execute the completion callback
            console.error(`[CLI Prompter] Would execute completion callback: ${onComplete}`);
        }
        return {
            content: [{
                    type: "text",
...
async function ${name.replace(/[^a-zA-Z0-9]/g, '_')}() {
  console.error('${description || name}');
...
yargs(hideBin(process.argv))
  ...
  .command('${name}', '${description || name}', (yargs) => {
    ...
    console.error('Running with options:', argv);
    // Implementation here
  })
  .help()
  .argv;
 ...
 */
async function handleValidateInput(args) {
    try {
