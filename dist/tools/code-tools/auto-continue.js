...
function activate() {
    console.error("[TOOL] auto-continue activated");
}
/**
 * Handles file write events for continuation management
 */
function onFileWrite(event) {
    console.error(`[Auto Continue] Watching file write: ${event.path}`);
    ...
    if (path.basename(event.path).includes('continue') || event.content.includes('// CONTINUE_MARKER')) {
        console.error(`[Auto Continue] Detected continuation file: ${event.path}`);
...
function onSessionStart(session) {
    console.error(`[Auto Continue] Session started: ${session.id}`);
 ...
 */
async function handleRegisterContinuation(args) {
    try {
 ...
 */
async function handleCheckContinuation(args) {
    try {
 ...
 */
async function handleResumeContinuation(args) {
    try {
 ...
 */
async function handleCancelContinuation(args) {
    try {
