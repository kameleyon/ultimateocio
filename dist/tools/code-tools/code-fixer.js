...
exports.FixProjectSchema = exports.AnalyzeCodeSchema = exports.FixCodeSchema = void 0;
exports.activate = activate;
exports.onFileWrite = onFileWrite;
exports.onSessionStart = onSessionStart;
exports.onCommand = onCommand;
exports.handleFixCode = handleFixCode;
exports.handleAnalyzeCode = handleAnalyzeCode;
exports.handleFixProject = handleFixProject;
function activate() {
    console.error("[TOOL] code-fixer activated (vigilant mode)");
    checkDependencies();
}
/**
 * Watches for file writes and analyzes for code issues
 */
async function onFileWrite(event) {
    const language = getLanguageFromExtension(event.path);
    if (!language)
        return;
    console.error(`[Code Fixer] Watching file write: ${event.path}`);
    const issues = analyzeCode(event.content, language);
    if (issues.length > 0) {
        console.error(`[Code Fixer] Detected ${issues.length} issue(s) in ${event.path}`);
...
function onSessionStart(session) {
    console.error(`[Code Fixer] Session started: ${session.id}`);
...
async function onCommand(command) {
    switch (command.name) {
        case 'code-fixer:analyze':
            console.error('[Code Fixer] Analyzing file...');
            // You would pass args[0] with path/content
            break;
        case 'code-fixer:fix':
            console.error('[Code Fixer] Fixing file...');
            // You would call fixCode here with proper params
            break;
        case 'code-fixer:report':
            console.error('[Code Fixer] Generating report...');
            break;
        default:
...
function checkDependencies() {
    ...
    for (const pkg of required) {
        try {
            ...
            console.error(`[Dependency] Missing "${pkg}". Installing...`);
            try {
                (0, child_process_1.execSync)(`npm install ${pkg}`, { stdio: 'inherit' });
                console.error(`[Dependency] "${pkg}" installed successfully.`);
            }
            catch (err) {
...
exports.FixCodeSchema = zod_1.z.object({
    path: zod_1.z.string(),
...
exports.FixProjectSchema = zod_1.z.object({
 ...
 */
async function handleFixProject(args) {
 ...
 * Fixes code issues
 */
function fixCode(content, language, fixTypes) {
 ...
 */
async function processFile(filePath, fixTypes) {
    try {
        // Determine the language
 ...
 */
async function getFilesInDirectory(directory, recursive, extensions) {
    const files = [];
 ...
 */
async function processLargeFile(filePath, language, fixTypes) {
    return new Promise((resolve, reject) => {
        const chunks = [];
        const fixes = [];
        const readStream = (0, fs_1.createReadStream)(filePath, { encoding: 'utf8', highWaterMark: CONFIG.CHUNK_SIZE });
