...
function activate() {
    console.error("[TOOL] duplicate-finder activated");
}
/**
 * Handles file write events to detect potential duplicates
 */
function onFileWrite(event) {
    console.error(`[Duplicate Finder] Watching file write: ${event.path}`);
    // Could analyze file for duplicate content
    const filename = path.basename(event.path);
    if (filename.match(/copy|duplicate|backup|\(\d+\)/i)) {
        console.error(`[Duplicate Finder] Detected potential duplicate file: ${event.path}`);
    }
}
/**
...
function onSessionStart(session) {
    console.error(`[Duplicate Finder] Session started: ${session.id}`);
 ...
 */
async function handleFindDuplicates(args) {
    try {
 ...
 */
async function handleCleanupDuplicates(args) {
    try {
