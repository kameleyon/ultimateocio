...
function activate() {
    console.error("[TOOL] plugin-loader activated");
...
async function initializeRegistry() {
    ...
    try {
        ...
        if (!fsSync.existsSync(pluginsDir)) {
            ...
            console.error(`[Plugin Loader] Created plugins directory: ${pluginsDir}`);
        }
        ...
        if (!fsSync.existsSync(configFile)) {
            ...
            console.error(`[Plugin Loader] Created plugin config file: ${configFile}`);
        }
            ...
            if (fsSync.existsSync(manifestPath)) {
                try {
                    ...
                    if (manifest.name && manifest.version && manifest.main) {
                        ...
                        console.error(`[Plugin Loader] Discovered plugin: ${pluginInfo.name} (${pluginInfo.version})`);
                    }
                }
                catch (error) {
...
async function savePluginConfig() {
    ...
    try {
        ...
        console.error(`[Plugin Loader] Saved plugin configuration to ${configFile}`);
    }
    catch (error) {
...
async function loadPlugin(pluginId) {
    ...
    if (!plugin.enabled) {
        console.error(`[Plugin Loader] Plugin is disabled: ${plugin.name}`);
        return false;
    }
    try {
        ...
        console.error(`[Plugin Loader] Successfully loaded plugin: ${plugin.name} (${plugin.version})`);
        return true;
    }
    catch (error) {
...
async function unloadPlugin(pluginId) {
    ...
    try {
        ...
        console.error(`[Plugin Loader] Successfully unloaded plugin: ${plugin.name}`);
        return true;
    }
    catch (error) {
        // Update plugin with error
...
function onFileWrite(event) {
    ...
    if (event.path.startsWith(pluginsDir)) {
        console.error(`[Plugin Loader] Plugin file changed: ${event.path}`);
        ...
        if (pluginId) {
            ...
            if (plugin && plugin.entryPoint === event.path) {
                console.error(`[Plugin Loader] Plugin main file changed: ${plugin.name}`);
...
function onSessionStart(session) {
    console.error(`[Plugin Loader] Session started: ${session.id}`);
    // Call hook on plugins
 ...
 */
async function handleInstallPlugin(args) {
    try {
 ...
 */
async function installPluginFromNpm(packageName, options) {
    ...
    console.error(`[Plugin Loader] Installing npm package: ${packageName}`);
    ...
    await fs.writeFile(path.join(packageDir, 'package.json'), JSON.stringify(manifest, null, 2), 'utf8');
  ...
  deactivate: function() {
    console.error('Plugin deactivated: ${packageName}');
  }
};`, 'utf8');
    // Create plugin info
    const pluginInfo = {
 ...
 */
async function installPluginFromGit(repository, options) {
    // In a real implementation, this would clone the git repository
    // For the boilerplate, we'll just simulate the installation
    console.error(`[Plugin Loader] Installing from git: ${repository}`);
    ...
    await fs.writeFile(path.join(repoDir, 'package.json'), JSON.stringify(manifest, null, 2), 'utf8');
    // Create a simple index.js
    await fs.writeFile(path.join(repoDir, 'index.js'), `module.exports = {
  activate: function(api) {
    api.log('Plugin activated: ${repoName}');
  },
  deactivate: function() {
    console.error('Plugin deactivated: ${repoName}');
  }
};`, 'utf8');
    // Create plugin info
    const pluginInfo = {
 ...
 */
async function installPluginFromDirectory(directory, options) {
    console.error(`[Plugin Loader] Installing from directory: ${directory}`);
    ...
    if (fsSync.existsSync(mainPath)) {
        ...
        await fs.writeFile(path.join(pluginDir, path.basename(manifest.main)), `module.exports = {
  activate: function(api) {
    api.log('Plugin activated: ${manifest.name}');
  },
  deactivate: function() {
    console.error('Plugin deactivated: ${manifest.name}');
  }
};`, 'utf8');
    }
    // Create plugin info
 ...
 */
async function installPluginFromZip(zipFile, options) {
    // In a real implementation, this would extract the zip file
    // For the boilerplate, we'll just simulate the installation
    console.error(`[Plugin Loader] Installing from zip: ${zipFile}`);
    ...
    await fs.writeFile(path.join(extractDir, 'package.json'), JSON.stringify(manifest, null, 2), 'utf8');
    // Create a simple index.js
    await fs.writeFile(path.join(extractDir, 'index.js'), `module.exports = {
  activate: function(api) {
    api.log('Plugin activated: ${zipName}');
  },
  deactivate: function() {
    console.error('Plugin deactivated: ${zipName}');
  }
};`, 'utf8');
    // Create plugin info
    const pluginInfo = {
 ...
 */
async function handleUninstallPlugin(args) {
    try {
        ...
        if (pluginDir && fsSync.existsSync(pluginDir) && pluginDir.startsWith(registry.getPluginsDir())) {
            try {
                ...
                console.error(`[Plugin Loader] Removing plugin directory: ${pluginDir}`);
 ...
 */
async function handleEnablePlugin(args) {
    try {
 ...
 */
async function handleDisablePlugin(args) {
    try {
        const result = exports.EnableDisablePluginSchema.safeParse(args);
        if (!result.success) {
            return {
 ...
 */
async function handleListPlugins(args) {
    try {
 ...
 */
async function handleGetPluginInfo(args) {
    try {
