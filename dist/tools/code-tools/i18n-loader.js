...
function activate() {
    console.error("[TOOL] i18n-loader activated");
...
function onFileWrite(event) {
    ...
    if (event.path.includes('/locales/') || event.path.includes('/translations/')) {
        console.error(`[i18n Loader] Translation file changed: ${event.path}`);
        ...
        if (match) {
            const lang = match[1];
            console.error(`[i18n Loader] Reloading language: ${lang}`);
            // Reload language
...
function onSessionStart(session) {
    console.error(`[i18n Loader] Session started: ${session.id}`);
 ...
 */
async function handleLoadLanguage(args) {
    try {
 ...
 */
async function handleSwitchLanguage(args) {
    try {
