...
const eventHistory = [];
function activate() {
    console.error("[TOOL] event-dispatcher activated");
...
function onFileWrite(event) {
    console.error(`[Event Dispatcher] File write detected: ${event.path}`);
...
function onSessionStart(session) {
    console.error(`[Event Dispatcher] Session started: ${session.id}`);
 ...
 */
async function handleDispatchEvent(args) {
    try {
 ...
 */
async function handleSubscribeEvent(args) {
    try {
        ...
        const listener = (data) => {
            ...
            // Execute callback
            if (callback) {
                console.error(`[Event Dispatcher] Executing callback for event '${event}': ${callback}`);
 ...
 */
async function handleRegisterEvent(args) {
    try {
