...
function activate() {
    console.error("[TOOL] cli-toolkit activated");
}
/**
 * Handles file write events that might trigger CLI updates
 */
function onFileWrite(event) {
    console.error(`[CLI Toolkit] Watching file write: ${event.path}`);
    ...
    if (fileExt === '.sh' || fileExt === '.bat' || fileExt === '.cmd' || fileExt === '.ps1') {
        console.error(`[CLI Toolkit] Detected script file change: ${event.path}`);
    }
}
/**
...
function onSessionStart(session) {
    console.error(`[CLI Toolkit] Session started: ${session.id}`);
}
/**
    ...
    try {
        ...
        try {
            ...
            if (captureOutput) {
                ...
                (0, child_process_1.execSync)(fullCommand, { ...options, stdio: 'inherit' });
 ...
 */
async function handleChainCommands(args) {
    try {
