...
function activate() {
    console.error("[TOOL] theme-switcher activated");
    ...
    try {
        if (fsSync.existsSync(CONFIG_FILE_PATH)) {
            ...
            currentConfig = {
                ...
                ].filter((mode, index, self) => index === self.findIndex(m => m.name === mode.name)),
                layouts: [
                    ...DEFAULT_CONFIG.layouts,
                    ...(loadedConfig.layouts || []),
                ].filter((layout, index, self) => index === self.findIndex(l => l.name === layout.name)),
            };
            console.error(`[Theme Switcher] Loaded configuration from ${CONFIG_FILE_PATH}`);
        }
        else {
            // Create default config file
            await saveConfig();
            console.error(`[Theme Switcher] Created default configuration at ${CONFIG_FILE_PATH}`);
        }
    }
    catch (error) {
...
async function saveConfig() {
    try {
        ...
        console.error(`[Theme Switcher] Saved configuration to ${CONFIG_FILE_PATH}`);
    }
    catch (error) {
...
function onFileWrite(event) {
    ...
    if (path.basename(event.path) === path.basename(CONFIG_FILE_PATH)) {
        console.error(`[Theme Switcher] Theme configuration file changed: ${event.path}`);
        loadConfig();
    }
}
/**
...
function onSessionStart(session) {
    console.error(`[Theme Switcher] Session started: ${session.id}`);
 ...
 */
async function handleSetLayout(args) {
    try {
 ...
 */
async function handleAddTheme(args) {
    try {
 ...
 */
async function handleAddLayout(args) {
    try {
 ...
 */
async function handleSetAnimations(args) {
    try {
 ...
 */
async function handleGenerateCss(args) {
    try {
