...
exports.AnalyzeComponentSchema = exports.AddComponentPartSchema = exports.ListTemplatesSchema = exports.CreateComponentSchema = void 0;
...
function detectProjectFramework() {
    try {
        ...
        if (fsSync.existsSync('package.json')) {
            ...
            // Detect TypeScript
            detectedFramework.typescript = !!(dependencies.typescript ||
    ...
    return __awaiter(this, void 0, void 0, function* () {
        switch (command.name) {
            case 'component-builder:create':
                ...
                return yield handleCreateComponent(command.args[0]);
            ...
            case 'component-builder:add-part':
                ...
                return yield handleAddComponentPart(command.args[0]);
            case 'component-builder:analyze':
                console.log('[Component Builder] Analyzing component...');
                return yield handleAnalyzeComponent(command.args[0]);
            default:
...
exports.CreateComponentSchema = zod_1.z.object({
    name: zod_1.z.string(),
    framework: zod_1.z.enum(['react', 'vue', 'angular', 'svelte', 'solid']).optional(),
    type: zod_1.z.enum(['functional', 'class', 'hooks', 'styled', 'hoc']).optional(),
    outputDir: zod_1.z.string().optional(),
    includeStyles: zod_1.z.boolean().optional().default(true),
    includeTests: zod_1.z.boolean().optional().default(false),
    includeStories: zod_1.z.boolean().optional().default(false),
    customTemplate: zod_1.z.string().optional(),
    customProps: zod_1.z.record(zod_1.z.any()).optional(),
});
...
exports.AddComponentPartSchema = zod_1.z.object({
    componentPath: zod_1.z.string(),
    part: zod_1.z.enum(['styles', 'tests', 'stories', 'types']),
    content: zod_1.z.string().optional(),
    overwrite: zod_1.z.boolean().optional().default(false),
});
exports.AnalyzeComponentSchema = zod_1.z.object({
    path: zod_1.z.string(),
    detectDependencies: zod_1.z.boolean().optional().default(true),
    suggestImprovements: zod_1.z.boolean().optional().default(true),
});
/**
 * Helper to convert component name to different case formats
 */
function convertCase(name, format) {
...
function createComponent(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const replacements = Object.assign({ 'name': options.name, 'name-kebab': convertCase(options.name, 'kebab'), 'name-camel': convertCase(options.name, 'camel'), 'name-snake': convertCase(options.name, 'snake') }, options.customProps);
        // Process templates
        const componentContent = processTemplate(template.content, replacements);
        // Ensure output directory exists
        yield fs.mkdir(options.outputDir, { recursive: true });
        ...
        yield fs.writeFile(componentFilePath, componentContent);
  ...
  <div class="${options.name}__content">
    ...
    <button (click)="increment()">Increment</button>
            ...
            yield fs.writeFile(templatePath, templateContent);
        ...
        if (options.includeStyles && !isInlineStyles && template.styles) {
            ...
            yield fs.writeFile(stylesPath, stylesContent);
        ...
        if (options.includeTests && template.tests) {
            ...
            yield fs.writeFile(testsPath, testsContent);
        ...
        if (options.includeStories) {
            ...
            yield fs.writeFile(storyPath, storyContent);
 ...
 */
function createStoryContent(componentName, framework) {
    switch (framework) {
        case 'react':
...
function handleCreateComponent(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.CreateComponentSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const { name, framework = detectedFramework.framework, type = 'functional', outputDir, includeStyles, includeTests, includeStories, customTemplate, customProps } = result.data;
            ...
            // Create component
            const { path: componentPath, files } = yield createComponent({
                name,
                framework,
                type,
 ...
 */
function handleAddComponentPart(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.AddComponentPartSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            if (componentExt === '.vue') {
                ...
                const componentContent = yield fs.readFile(componentPath, 'utf8');
...
.${componentName}__content {
  ...
  children?: React.ReactNode;
            ...
            // Write the part file
            yield fs.writeFile(partPath, partContent);
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
 ...
 */
function createTestContent(componentName, framework) {
    switch (framework) {
        case 'react':
            return `import React from 'react';
import { render, screen } from '@testing-library/react';
 ...
 */
function handleAnalyzeComponent(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.AnalyzeComponentSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
