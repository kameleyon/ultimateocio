...
exports.CustomizeMockDataSchema = exports.ListTemplatesSchema = exports.InsertMockDataSchema = exports.GenerateMockDataSchema = void 0;
 ...
 */
function handleGenerateMockData(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.GenerateMockDataSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            if (outputPath) {
                try {
                    ...
                    // Write to file
                    yield fs.writeFile(outputPath, JSON.stringify(mockData, null, 2));
                }
                catch (error) {
                    return {
                        content: [{
 ...
 */
function handleInsertMockData(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.InsertMockDataSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
...
function handleListTemplates(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.ListTemplatesSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
 ...
 */
function handleCustomizeMockData(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.CustomizeMockDataSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const customizedData = mockData.map(item => (Object.assign(Object.assign({}, item), overrides)));
            // Write to file if output path is provided
            if (outputPath) {
                try {
                    // Create directory if it doesn't exist
                    ...
                    // Write to file
                    yield fs.writeFile(outputPath, JSON.stringify(customizedData, null, 2));
                }
                catch (error) {
                    return {
                        content: [{
...
exports.GenerateMockDataSchema = zod_1.z.object({
    type: zod_1.z.enum([
        'user', 'product', 'post', 'comment', 'address',
        'transaction', 'event', 'task', 'contact', 'custom'
    ]),
    count: zod_1.z.number().optional().default(10),
    customSchema: zod_1.z.record(zod_1.z.any()).optional(),
    locale: zod_1.z.string().optional().default('en-US'),
    seed: zod_1.z.string().optional(),
    outputPath: zod_1.z.string().optional(),
});
exports.InsertMockDataSchema = zod_1.z.object({
    path: zod_1.z.string(),
    ...
    count: zod_1.z.number().optional().default(10),
    format: zod_1.z.enum(['json', 'js', 'ts']).optional().default('json'),
...
exports.CustomizeMockDataSchema = zod_1.z.object({
    template: zod_1.z.enum([
        'user', 'product', 'post', 'comment', 'address',
        'transaction', 'event', 'task', 'contact', 'custom'
    ]),
    overrides: zod_1.z.record(zod_1.z.any()),
 ...
 */
function getRandomItem(array, random = Math.random) {
    return array[Math.floor(random() * array.length)];
}
/**
 * Generate user mock data
 */
function generateUserData(count, random = Math.random) {
    const firstNames = [
    ...
    for (let i = 0; i < count; i++) {
        ...
        users.push({
            ...
            username,
            email,
            birthDate: new Date(getRandomInt(1960, 2000, random), getRandomInt(0, 11, random), getRandomInt(1, 28, random)).toISOString().split('T')[0],
            address: {
 ...
 */
function generateProductData(count, random = Math.random) {
    const categories = [
    ...
    for (let i = 0; i < count; i++) {
        ...
        result.push({
            ...
            tags: Array.from({ length: getRandomInt(1, 4, random) }, () => getRandomItem(['new', 'sale', 'popular', 'featured', 'trending', 'seasonal', 'limited'], random)),
 ...
 */
function generatePostData(count, random = Math.random) {
    const titles = [
    ...
    for (let i = 0; i < count; i++) {
        ...
        posts.push({
            ...
            tags: Array.from({ length: getRandomInt(1, 5, random) }, () => getRandomItem(['webdev', 'tutorial', 'tips', 'guide', 'howto', 'review', 'opinion', 'news'], random)),
 ...
 */
function generateMockData(type, count, customSchema, seed, locale = 'en-US') {
 ...
 */
function generateCommentData(count, random = Math.random) {
    const comments = [];
    for (let i = 0; i < count; i++) {
        const start = getRandomInt(0, loremIpsum.length - 100, random);
 ...
 */
function generateAddressData(count, random = Math.random) {
 ...
 */
function generateTransactionData(count, random = Math.random) {
    const transactions = [];
    for (let i = 0; i < count; i++) {
 ...
 */
function generateEventData(count, random = Math.random) {
    const events = [];
    const now = new Date();
    const year = now.getFullYear();
    const month = now.getMonth();
    for (let i = 0; i < count; i++) {
        const startDate = new Date(year, month, getRandomInt(1, 28, random), getRandomInt(9, 19, random), getRandomInt(0, 59, random));
        ...
        events.push({
            ...
            attendees: Array.from({ length: getRandomInt(1, 10, random) }, () => getRandomInt(1, 50, random)),
 ...
 */
function generateTaskData(count, random = Math.random) {
    const tasks = [];
    for (let i = 0; i < count; i++) {
        tasks.push({
            id: i + 1,
            ...
            tags: Array.from({ length: getRandomInt(0, 3, random) }, () => getRandomItem(['bug', 'feature', 'documentation', 'design', 'backend', 'frontend'], random)),
 ...
 */
function generateContactData(count, random = Math.random) {
    const contacts = [];
    const firstNames = [
        'James', 'John', 'Robert', 'Michael', 'William',
        'Mary', 'Patricia', 'Jennifer', 'Linda', 'Elizabeth'
    ...
    for (let i = 0; i < count; i++) {
        ...
        contacts.push({
            ...
            tags: Array.from({ length: getRandomInt(0, 3, random) }, () => getRandomItem(['client', 'lead', 'vendor', 'partner', 'personal'], random)),
            createdAt: new Date(Date.now() - getRandomInt(1, 365, random) * 24 * 60 * 60 * 1000).toISOString()
        });
    }
    return contacts;
 ...
 */
function generateCustomData(schema, count, random = Math.random) {
    const result = [];
    for (let i = 0; i < count; i++) {
        const item = {};
        ...
        for (const [key, def] of Object.entries(schema)) {
            if (typeof def === 'string') {
                ...
                if ('type' in def && typeof def.type === 'string' && def.type === 'enum' && 'values' in def && Array.isArray(def.values)) {
                    ...
                    item[key] = Array.from({ length: length }, () => ('of' in def && typeof def.of === 'string') ? generateValueForType(def.of, random) : getRandomItem(def.of, random));
                }
 ...
 */
function generateValueForType(type, random = Math.random) {
    switch (type) {
        case 'string':
