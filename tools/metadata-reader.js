...
exports.ParseFileContentSchema = exports.BatchProcessSchema = exports.ExtractMetadataSchema = exports.ReadMetadataSchema = void 0;
...
function checkExternalTools() {
    // Check for exiftool
    try {
        (0, child_process_1.execSync)('exiftool -ver', { stdio: 'ignore' });
    ...
    try {
        (0, child_process_1.execSync)('convert -version', { stdio: 'ignore' });
...
exports.ReadMetadataSchema = zod_1.z.object({
    path: zod_1.z.string(),
    type: zod_1.z.enum(['auto', 'file', 'image', 'audio', 'video', 'document', 'code', 'archive'])
        .optional().default('auto'),
    includeBasic: zod_1.z.boolean().optional().default(true),
    includeExtended: zod_1.z.boolean().optional().default(false),
});
exports.ExtractMetadataSchema = zod_1.z.object({
    path: zod_1.z.string(),
    options: zod_1.z.object({
        extractExif: zod_1.z.boolean().optional().default(true),
        extractContent: zod_1.z.boolean().optional().default(false),
        extractTags: zod_1.z.boolean().optional().default(true),
        extractDimensions: zod_1.z.boolean().optional().default(true),
        extractDuration: zod_1.z.boolean().optional().default(true),
    }).optional().default({}),
    useExternalTools: zod_1.z.boolean().optional().default(true),
});
exports.BatchProcessSchema = zod_1.z.object({
    directory: zod_1.z.string(),
    recursive: zod_1.z.boolean().optional().default(false),
    pattern: zod_1.z.string().optional(),
    outputFormat: zod_1.z.enum(['json', 'csv']).optional().default('json'),
    outputPath: zod_1.z.string().optional(),
});
exports.ParseFileContentSchema = zod_1.z.object({
    path: zod_1.z.string(),
    format: zod_1.z.enum(['auto', 'json', 'yaml', 'xml', 'csv', 'toml', 'ini', 'markdown'])
        .optional().default('auto'),
    extractStructure: zod_1.z.boolean().optional().default(true),
    extractLinks: zod_1.z.boolean().optional().default(false),
    extractHeadings: zod_1.z.boolean().optional().default(false),
});
/**
 ...
 */
function getBasicMetadata(filePath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
 ...
 */
function getExtendedMetadata(filePath_1, type_1) {
    return __awaiter(this, arguments, void 0, function* (filePath, type, useExternalTools = true) {
        const metadata = {};
        try {
            if (type === 'image') {
                if (useExternalTools && externalTools.exiftool) {
                    try {
                        const exifData = (0, child_process_1.execSync)(`exiftool -j "${filePath}"`, { encoding: 'utf8' });
                ...
                if (useExternalTools && externalTools.imagemagick) {
                    try {
                        const imageMagickData = (0, child_process_1.execSync)(`identify -format "%[width]x%[height] %[colorspace] %m" "${filePath}"`, { encoding: 'utf8' });
                ...
                if (useExternalTools && externalTools.ffmpeg) {
                    try {
                        const ffmpegData = (0, child_process_1.execSync)(`ffprobe -v quiet -print_format json -show_format -show_streams "${filePath}"`, { encoding: 'utf8' });
 ...
 */
function handleReadMetadata(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.ReadMetadataSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const metadata = {};
            if (includeBasic) {
                Object.assign(metadata, yield getBasicMetadata(filePath));
            }
            // Determine file type
            ...
            if (includeExtended) {
                const extended = yield getExtendedMetadata(filePath, fileType);
 ...
 */
function handleExtractMetadata(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.ExtractMetadataSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            // Get basic metadata
            const basicMetadata = yield getBasicMetadata(filePath);
            ...
            if ((fileType === 'image' || fileType === 'video') && options.extractDimensions) {
                if (fileType === 'image' && useExternalTools && externalTools.imagemagick) {
                    try {
                        const imageMagickData = (0, child_process_1.execSync)(`identify -format "%[width]x%[height]" "${filePath}"`, { encoding: 'utf8' });
                    ...
                    try {
                        const ffmpegData = (0, child_process_1.execSync)(`ffprobe -v quiet -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 "${filePath}"`, { encoding: 'utf8' });
            ...
            if ((fileType === 'audio' || fileType === 'video') && options.extractDuration) {
                if (useExternalTools && externalTools.ffmpeg) {
                    try {
                        const durationData = (0, child_process_1.execSync)(`ffprobe -v quiet -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${filePath}"`, { encoding: 'utf8' });
 ...
 */
function handleBatchProcess(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.BatchProcessSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            // Find all files
            const filePaths = [];
            yield findFiles(directory, filePaths, {
                recursive,
            ...
            for (const filePath of filePaths) {
                try {
                    const basicMetadata = yield getBasicMetadata(filePath);
                    const fileType = getFileType(filePath);
                    results.push(Object.assign(Object.assign({}, basicMetadata), { fileType }));
                }
                catch (error) {
            ...
            if (outputPath) {
                try {
                    ...
                    if (outputFormat === 'json') {
                        yield fs.writeFile(outputPath, JSON.stringify(results, null, 2));
                    }
                    else if (outputFormat === 'csv') {
                        const csv = convertToCSV(results);
                        yield fs.writeFile(outputPath, csv);
                    }
                }
                catch (error) {
 ...
 */
function findFiles(dir, results, options) {
    return __awaiter(this, void 0, void 0, function* () {
        const entries = yield fs.readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
            const fullPath = path.join(dir, entry.name);
            if (entry.isDirectory() && options.recursive) {
                yield findFiles(fullPath, results, options);
            }
 ...
 */
function handleParseFileContent(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const parseResult = exports.ParseFileContentSchema.safeParse(args);
            ...
            const { path: filePath, format, extractStructure, extractLinks, extractHeadings } = parseResult.data;
            // Check if file exists
            try {
                yield fs.access(filePath);
            }
            ...
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify(Object.assign(Object.assign({}, parsedData), { message: `Successfully parsed ${path.basename(filePath)} as ${detectedFormat}` }), null, 2)
                    }]
            };
        }
        catch (error) {
 ...
 */
function extractLinksFromContent(content, format) {
    const links = [];
 ...
 */
function extractHeadingsFromContent(content, format) {
    const headings = [];
    if (format === 'markdown') {
