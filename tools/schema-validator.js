...
exports.ConvertSchemaSchema = exports.GenerateSchemaSchema = exports.AnalyzeSchemaSchema = exports.CompileSchemaSchema = exports.ValidateSchemaSchema = void 0;
...
exports.ValidateSchemaSchema = zod_1.z.object({
    schemaPath: zod_1.z.string().optional(),
    schemaContent: zod_1.z.string().optional(),
    schemaFormat: zod_1.z.enum(['json', 'yaml', 'typescript', 'graphql', 'avro']).optional().default('json'),
    dataPath: zod_1.z.string().optional(),
    dataContent: zod_1.z.string().optional(),
    dataFormat: zod_1.z.enum(['json', 'yaml', 'csv', 'txt']).optional().default('json'),
    options: zod_1.z.object({
        allowUnknownProperties: zod_1.z.boolean().optional(),
        coerceTypes: zod_1.z.boolean().optional(),
        removeAdditional: zod_1.z.boolean().optional(),
        strictArrays: zod_1.z.boolean().optional(),
        strictObjects: zod_1.z.boolean().optional(),
    }).optional().default({}),
}).refine(data => data.schemaPath !== undefined || data.schemaContent !== undefined, {
    message: 'Either schemaPath or schemaContent must be provided',
    path: ['schema'],
}).refine(data => data.dataPath !== undefined || data.dataContent !== undefined, {
    message: 'Either dataPath or dataContent must be provided',
    path: ['data'],
});
exports.CompileSchemaSchema = zod_1.z.object({
    schemaPath: zod_1.z.string().optional(),
    schemaContent: zod_1.z.string().optional(),
    schemaFormat: zod_1.z.enum(['json', 'yaml', 'typescript', 'graphql', 'avro']).optional().default('json'),
    outputFormat: zod_1.z.enum(['json', 'typescript']).optional().default('json'),
    outputPath: zod_1.z.string().optional(),
}).refine(data => data.schemaPath !== undefined || data.schemaContent !== undefined, {
    message: 'Either schemaPath or schemaContent must be provided',
    path: ['schema'],
});
exports.AnalyzeSchemaSchema = zod_1.z.object({
    schemaPath: zod_1.z.string().optional(),
    schemaContent: zod_1.z.string().optional(),
    schemaFormat: zod_1.z.enum(['json', 'yaml', 'typescript', 'graphql', 'avro']).optional().default('json'),
    includeStats: zod_1.z.boolean().optional().default(true),
    includeValidation: zod_1.z.boolean().optional().default(true),
}).refine(data => data.schemaPath !== undefined || data.schemaContent !== undefined, {
    message: 'Either schemaPath or schemaContent must be provided',
    path: ['schema'],
});
exports.GenerateSchemaSchema = zod_1.z.object({
    dataPath: zod_1.z.string().optional(),
    dataContent: zod_1.z.string().optional(),
    dataFormat: zod_1.z.enum(['json', 'yaml', 'csv', 'txt']).optional().default('json'),
    outputFormat: zod_1.z.enum(['json', 'typescript']).optional().default('json'),
    ...
    options: zod_1.z.object({
        ...
        examples: zod_1.z.boolean().optional().default(true),
        descriptions: zod_1.z.boolean().optional().default(true),
    }).optional().default({}),
}).refine(data => data.dataPath !== undefined || data.dataContent !== undefined, {
    message: 'Either dataPath or dataContent must be provided',
    path: ['data'],
});
exports.ConvertSchemaSchema = zod_1.z.object({
    schemaPath: zod_1.z.string().optional(),
    schemaContent: zod_1.z.string().optional(),
    sourceFormat: zod_1.z.enum(['json', 'yaml', 'typescript', 'graphql', 'avro']).optional().default('json'),
    targetFormat: zod_1.z.enum(['json', 'typescript']).default('json'),
    outputPath: zod_1.z.string().optional(),
}).refine(data => data.schemaPath !== undefined || data.schemaContent !== undefined, {
    message: 'Either schemaPath or schemaContent must be provided',
    path: ['schema'],
...
function isSchemaFile(filePath) {
    ...
    return ext === '.json' && (fileName.includes('schema') ||
        fileName.includes('swagger') ||
        fileName.includes('openapi') ||
        fileName.endsWith('.schema.json')) || ext === '.graphql' ||
        ext === '.gql' ||
        ext === '.avsc' ||
 ...
 */
function parseSchema(schemaPath, schemaContent, schemaFormat) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (schemaContent) {
            ...
            try {
                content = yield fs.readFile(schemaPath, 'utf8');
            }
            catch (error) {
 ...
 */
function convertTsTypeToJsonSchemaType(tsType) {
 ...
 */
function convertGraphQLTypeToJsonSchemaType(graphQLType) {
 ...
 */
function parseData(dataPath, dataContent, dataFormat) {
    return __awaiter(this, void 0, void 0, function* () {
        // Get data content
        let content;
        if (dataContent) {
            ...
            try {
                content = yield fs.readFile(dataPath, 'utf8');
            }
            catch (error) {
 ...
 */
function validateAgainstSchema(schema, data, options = defaultValidatorOptions) {
    ...
    function validate(schemaNode, dataNode, path = '') {
        ...
        if (schemaNode.type) {
            ...
            if ((schemaNode.type === 'string' && dataType !== 'string') ||
 ...
 */
function getSchemaInfo(schema, format) {
    const info = {
        format,
 ...
 */
function handleValidateSchema(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.ValidateSchemaSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const { schemaPath, schemaContent, schemaFormat, dataPath, dataContent, dataFormat, options } = result.data;
            // Create cache key
            ...
            const schema = yield parseSchema(schemaPath, schemaContent, schemaFormat);
            const data = yield parseData(dataPath, dataContent, dataFormat);
            // Merge options with defaults
            const validationOptions = Object.assign(Object.assign({}, defaultValidatorOptions), options);
 ...
 */
function handleCompileSchema(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.CompileSchemaSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const { schemaPath, schemaContent, schemaFormat, outputFormat, outputPath } = result.data;
            // Parse schema
            const schema = yield parseSchema(schemaPath, schemaContent, schemaFormat);
            let compiledSchema;
            ...
            if (outputPath) {
                try {
                    ...
                    yield fs.writeFile(outputPath, compiledSchema, 'utf8');
                }
                catch (error) {
 ...
 */
function generateTypeScriptFromSchema(schema) {
 ...
 */
function handleAnalyzeSchema(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.AnalyzeSchemaSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const { schemaPath, schemaContent, schemaFormat, includeStats, includeValidation } = result.data;
            // Parse schema
            const schema = yield parseSchema(schemaPath, schemaContent, schemaFormat);
            // Get schema info
 ...
 */
function analyzeSchemaStats(schema) {
 ...
 */
function handleGenerateSchema(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.GenerateSchemaSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const { dataPath, dataContent, dataFormat, outputFormat, outputPath, options } = result.data;
            // Parse data
            const data = yield parseData(dataPath, dataContent, dataFormat);
            ...
            if (outputPath) {
                try {
                    ...
                    yield fs.writeFile(outputPath, outputContent, 'utf8');
                }
                catch (error) {
 ...
 */
function handleConvertSchema(args) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const result = exports.ConvertSchemaSchema.safeParse(args);
            if (!result.success) {
                return {
                    content: [{
                            type: "text",
            ...
            const { schemaPath, schemaContent, sourceFormat, targetFormat, outputPath } = result.data;
            // Parse schema
            const schema = yield parseSchema(schemaPath, schemaContent, sourceFormat);
            let convertedSchema;
            ...
            if (outputPath) {
                try {
                    ...
                    yield fs.writeFile(outputPath, convertedSchema, 'utf8');
                }
                catch (error) {
